...
- FFmpeg runtime：默认下载并打包 `iGmainC/encode-lab-ffmpeg` 发布的 `ffmpeg-8.1.1-rpu.1` runtime，其中固定包含 FFmpeg/FFprobe、x265 CLI 与 dovi_tool；运行时优先使用 bundled 路径，缺失时回退系统 PATH。
