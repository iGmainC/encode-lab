...
const TABS = [
  { id: "video", labelKey: "parameterInspector.tab.video" },
  { id: "audio", labelKey: "parameterInspector.tab.audio" },
  { id: "color", labelKey: "parameterInspector.tab.color" },
  { id: "output", labelKey: "parameterInspector.tab.output" },
] as const satisfies ReadonlyArray<{ id: InspectorTab; labelKey: string }>;
...
export function ParameterInspector({
  ...
  const dolbyVisionDetail = t(dolbyVisionStatus.detailKey, dolbyVisionStatus.detailParams);
  ...
  useEffect(() => {
    if (dolbyVisionStatus.resolved && !dolbyVisionStatus.available && draft.preserveDolbyVisionMetadata) {
      draft.setPreserveDolbyVisionMetadata(false);
    }
  }, [dolbyVisionStatus.available, dolbyVisionStatus.resolved, draft.preserveDolbyVisionMetadata, draft.setPreserveDolbyVisionMetadata]);
  ...
  useEffect(() => {
    ...
    if (draft.audioMode !== "copy") draft.setAudioMode("copy");
  }, [
    draft.audioMode,
  ...
  async function pickOutputDirectory() {
    ...
    try {
      ...
      setOutputActionError(t("parameterInspector.output.pickFailed", {
  ...
  function toggleDolbyVisionPreservation(checked: boolean) {
    if (checked) {
      ...
      // 音频复制是后端 DV 专用执行链路的硬约束，开关提交时同步归一化。
      draft.setAudioMode("copy");
    }
  ...
  function openTemplateSaveForm() {
    ...
    const defaultTaskName = draft.draftName.trim() || t("parameterInspector.template.newTask");
    setTemplateName(
      activeName && draft.hasNamedTemplate
        ? t("parameterInspector.template.copyName", { name: activeName })
        : t("parameterInspector.template.defaultName", { name: defaultTaskName }),
    );
    setTemplateTags("");
  ...
  async function saveCurrentTemplate() {
    ...
    if (!name) {
      setTemplateSaveError(t("parameterInspector.template.nameRequired"));
      return;
    }
    ...
    try {
      ...
      setTemplateSaveSuccess(t("parameterInspector.template.saved", { name }));
  ...
  return (
    <aside className="flex min-h-0 flex-col overflow-hidden rounded-lg border bg-card/80 xl:h-full" aria-label={t("parameterInspector.aria.inspector")}>
      <div className="grid grid-cols-4 border-b" role="tablist" aria-label={t("parameterInspector.aria.groups")}>
        {TABS.map((tab, index) => (
          <button
            key={tab.id}
      ...
      >
        ...
        {activeTab === "color" ? (
          <ColorInspector
            ...
            dolbyVisionDetail={dolbyVisionDetail}
      ...
      <div className="shrink-0 border-t bg-background/70 p-3">
        {issues.length > 0 ? (
          ...
          <div className="mb-3 flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400">
            ...
            {t("parameterInspector.validation.passed")}
          </div>
        )}

        {isSaveFormOpen ? (
          <div className="mb-3 space-y-2 rounded-md border bg-muted/15 p-2.5">
            <div className="text-xs font-semibold">{t("parameterInspector.template.saveTitle")}</div>
            <label className="grid gap-1 text-[11px] text-muted-foreground">
              <span>{t("parameterInspector.template.name")}</span>
              <input
                className={INPUT_CLASS}
                value={templateName}
                onChange={(event) => setTemplateName(event.target.value)}
                aria-label={t("parameterInspector.template.name")}
                autoFocus
              />
            </label>
            <label className="grid gap-1 text-[11px] text-muted-foreground">
              <span>{t("parameterInspector.template.tags")}</span>
              <input
                className={INPUT_CLASS}
                value={templateTags}
                onChange={(event) => setTemplateTags(event.target.value)}
                placeholder="web, archive, hdr"
                aria-label={t("parameterInspector.template.tagsAria")}
              />
            </label>
            {templateSaveError ? <div className="text-xs text-destructive" role="alert">{templateSaveError}</div> : null}
            <div className="flex items-center justify-end gap-2">
              <Button size="sm" variant="ghost" onClick={() => setIsSaveFormOpen(false)} disabled={isSavingTemplate}>{t("parameterInspector.template.cancel")}</Button>
              <Button size="sm" onClick={() => void saveCurrentTemplate()} disabled={isSavingTemplate}>
                {isSavingTemplate ? t("parameterInspector.template.saving") : t("parameterInspector.template.save")}
              </Button>
            </div>
          </div>
        ) : (
          <Button className="mb-2 w-full justify-start" size="sm" variant="ghost" onClick={openTemplateSaveForm}>
            <Save data-icon="inline-start" aria-hidden="true" />{t("parameterInspector.template.openSave")}
          </Button>
        )}
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
          <Button onClick={() => void onEnqueue()} disabled={blockingIssues.length > 0 || isEnqueuing}>
            ...
            {isEnqueuing ? t("parameterInspector.enqueue.running") : t("parameterInspector.enqueue.confirm")}
          </Button>
          <Button variant="secondary" onClick={onOpenTemplates} aria-label={t("parameterInspector.openTemplates")}>
...
function VideoInspector({
  ...
  return (
    <div className="space-y-3">
      <InspectorSection title={t("parameterInspector.video.encodingSettings")}>
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {!isStreamCopy ? (
            <>
              <InspectorField label="Encoder" hint={selectedEncoderCapability?.available ? t("parameterInspector.video.encoderAvailable") : t("parameterInspector.video.checkCapability")}>
                  ...
                  <SelectContent>
                    {filteredEncoders.map((encoder) => (
                      <SelectItem key={encoder.encoder} value={encoder.encoder} disabled={!encoder.available}>
                        {encoder.displayName}{encoder.available ? "" : t("parameterInspector.video.unavailableSuffix")}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <InspectorField label="Preset" hint={presetOptions.length === 0 ? t("parameterInspector.video.noPreset") : undefined}>
        ...
        {isStreamCopy ? (
          <DependencyHint tone="success">
            {t("parameterInspector.video.streamCopyHint")}
          </DependencyHint>
        ) : draft.formMode === "CRF" ? (
          <div className="rounded-md border bg-background/50 p-3">
            ...
            <Slider
              ...
              aria-label={t("parameterInspector.video.crfAria")}
            />
            <div className="mt-2 flex justify-between text-[11px] text-muted-foreground">
              <span>{t("parameterInspector.video.higherQuality")}</span><span>{t("parameterInspector.video.crfRecommended")}</span><span>{t("parameterInspector.video.smallerSize")}</span>
            </div>
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-3">
            <TextField label={t("parameterInspector.video.targetBitrate")} value={draft.formBitrateKbps} onChange={draft.setFormBitrateKbps} inputMode="numeric" />
        ...
        {!isStreamCopy ? (
          <ToggleRow
            label="2-pass"
            description={selectedEncoderCapability?.supportsTwoPass ? t("parameterInspector.video.twoPassSupported") : t("parameterInspector.video.twoPassUnsupported")}
      ...
      {!isStreamCopy ? (
        <InspectorSection title={t("parameterInspector.video.frameSpec")}>
          <ToggleRow
            label={t("parameterInspector.video.keepSourceSize")}
            description={draft.videoMetadata?.video?.width && draft.videoMetadata.video.height
              ? t("parameterInspector.video.sourceSize", { width: draft.videoMetadata.video.width, height: draft.videoMetadata.video.height })
              : t("parameterInspector.video.sourceSizePending")}
          ...
          <ToggleRow
            label={t("parameterInspector.video.keepSourceFps")}
            description={draft.videoMetadata?.video?.fps
              ? t("parameterInspector.video.sourceFps", { value: draft.videoMetadata.video.fps })
              : t("parameterInspector.video.sourceFpsPending")}
...
function AudioInspector() {
  ...
  return (
    <div className="space-y-3">
      <InspectorSection title={t("parameterInspector.audio.sourceTrack")}>
      ...
      <InspectorSection title={t("parameterInspector.audio.outputAudio")}>
        <InspectorField label="Mode">
          <Select
            value={draft.audioMode}
            onValueChange={(value) => draft.setAudioMode(value as "copy" | "custom")}
            disabled={draft.preserveDolbyVisionMetadata}
          >
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        {draft.audioMode === "custom" ? (
          <label className="grid gap-1.5 text-xs">
            ...
            />
            <span className="text-[11px] leading-4 text-muted-foreground">{t("parameterInspector.audio.customHint")}</span>
          </label>
        ) : (
          <DependencyHint tone="success">{t("parameterInspector.audio.copyHint")}</DependencyHint>
        )}
      </InspectorSection>
    </div>
  );
...
function ColorInspector({
  ...
  return (
    <div className="space-y-3">
      <InspectorSection title={t("parameterInspector.color.sourceInfo")}>
        <dl className="grid grid-cols-2 gap-3 text-xs">
      ...
      {isStreamCopy ? (
        <InspectorSection title={t("parameterInspector.color.copyBehavior")}>
          <DependencyHint tone="success">{t("parameterInspector.color.copyHint")}</DependencyHint>
        </InspectorSection>
      ) : (
        <>
          <InspectorSection title={t("parameterInspector.color.outputTags")}>
            ...
            <DependencyHint tone="warning">{t("parameterInspector.color.hdrSdrHint")}</DependencyHint>
          </InspectorSection>
          <InspectorSection title={t("parameterInspector.color.dolbyVisionSection")}>
            <ToggleRow
              label={t("parameterInspector.color.preserveDolbyVision")}
...
function OutputInspector({ onPickOutputDirectory }: { onPickOutputDirectory: () => Promise<void> }) {
  ...
  return (
    <div className="space-y-3">
      <InspectorSection title={t("parameterInspector.output.taskAndFiles")}>
        <TextField label={t("parameterInspector.output.taskName")} value={draft.draftName} onChange={draft.setDraftName} />
        <label className="grid gap-1.5 text-xs">
          <span className="text-muted-foreground">{t("parameterInspector.output.directory")}</span>
          <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-2">
            <input className={INPUT_CLASS} value={draft.outputDir} onChange={(event) => draft.setOutputDir(event.target.value)} placeholder={t("parameterInspector.output.directoryPlaceholder")} />
            <Button size="sm" variant="secondary" onClick={() => void onPickOutputDirectory()} disabled={!isTauriRuntime()} aria-label={t("parameterInspector.output.pickDirectory")}>
              <FolderOpen aria-hidden="true" />
            </Button>
          </div>
        </label>
        <TextField label={t("parameterInspector.output.fileNamePattern")} value={draft.fileNamePattern} onChange={draft.setFileNamePattern} />
        <div className="text-[11px] text-muted-foreground">{t("parameterInspector.output.fileNameHint")}</div>
      </InspectorSection>
      ...
      <InspectorSection title={t("parameterInspector.output.clipRange")}>
        <div className="grid grid-cols-2 gap-3">
          <NumberField label={t("parameterInspector.output.startSeconds")} value={draft.clipStartSec} onChange={draft.setClipStartSec} disabled={draft.preserveDolbyVisionMetadata} />
          <NumberField label={t("parameterInspector.output.endSeconds")} value={draft.clipEndSec} onChange={draft.setClipEndSec} disabled={draft.preserveDolbyVisionMetadata} />
        </div>
        <div className="text-[11px] text-muted-foreground">{t("parameterInspector.output.sourceDuration", { value: draft.videoMetadata?.durationSec?.toFixed(3) ?? "-" })}</div>
      </InspectorSection>
    </div>
  );
}
...
function Av1Inspector() {
  ...
  const { t } = useI18n();
  return (
    <InspectorSection title={t("parameterInspector.av1.title")} className="mt-3">
      {draft.formEncoder === "libaom-av1" ? (
        <>
          ...
          <ToggleRow label="row-mt" description={t("parameterInspector.av1.rowMt")} checked={draft.av1RowMt} onCheckedChange={draft.setAv1RowMt} />
          <div className="grid grid-cols-2 gap-3">
        ...
        <DependencyHint tone="success">{t("parameterInspector.av1.hardwareDefaults")}</DependencyHint>
      )}
    </InspectorSection>
  );
}
